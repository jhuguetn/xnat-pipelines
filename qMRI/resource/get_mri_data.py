...
def is_b1_scan(philips_scan_type_info, scan_type, scanID):

    type_tokens = ['b1']
    is_b1 = None

    if [scanID for ctype in type_tokens if ctype in scan_type.lower()] :
        # let's asume it is a B1 scan
        is_b1 = True        
    #else :
        # Philips stuff ??
...
def get_scans_list(connection,project,subjectID,experimentID,required_type):
    ...
    for scanID in scans :
        ...
        if scans[scanID]['quality'] == 'usable' :
            ...
            elif 'b1' == required_type.lower() and is_b1_scan(philips_scan_type_info,scans[scanID]['type'],scanID):
                process_scan_list.append(scanID)
