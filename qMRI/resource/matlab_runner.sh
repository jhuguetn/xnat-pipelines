...
if [ $# != 3 ]
  then
    ...
    #source /etc/profile.d/modules.sh; module load fsl; module load matlab; module load matlab/toolbox/spm8; module load matlab/toolbox/dipimage
    #RADIAN environment Setting:
    #source /etc/profile.d/modules.sh; module load fsl; module load matlab; module load matlab/toolbox/spm8; module load dipimage
    ...
    matlab -nodisplay -nosplash -r "try addpath(genpath('$TOOLBOXES_DIR')); addpath(genpath(spm('dir'))); xnat_fit_qmri_wrapper('$SORTED_SCANS_DIR','$OUTPUT_DIR'); catch; end; exit" -logfile $LOG_FILENAME
